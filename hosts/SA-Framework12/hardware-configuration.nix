...
in
{
  ...
  boot = {
    initrd = {
      ...
      # Same unlock hardening as SA-Framework16, minus TPM2 (not enrolled
      # here): make the FIDO2 "plug in"/"confirm presence" notices visible on
      # the console, and survive hard FIDO2 errors (e.g. a plugged-in but
      # still-locked OnlyKey) that would otherwise abort the unit and drop to
      # an emergency shell instead of offering the recovery key.
      systemd.services."systemd-cryptsetup@cryptroot" = {
        ...
        serviceConfig = {
          ...
          ExecStart = [
            ...
            "${initrdCryptsetup} attach 'cryptroot' '${luksDevice}' '-' 'tries=0'"
          ];
        };
      };
